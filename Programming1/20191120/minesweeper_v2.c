...
// ゲーム本体に関する関数
void initialization(int *mode, int *number_of_mines, int mine_map[MS_SIZE][MS_SIZE], int game_board[MS_SIZE][MS_SIZE], int *finish_flag, int *open_count);
void set_mines(int *number_of_mines, int mine_map[MS_SIZE][MS_SIZE]);
void mine_count(int mine_map[MS_SIZE][MS_SIZE]);
void select_cell(int *mode, int *x, int *y);
void check_neighbors(int x, int y, int mine_map[MS_SIZE][MS_SIZE], int game_board[MS_SIZE][MS_SIZE], int *open_count);
int main(void)

{
    ...
    while (replay_flag) {
        ...
        // 各種数値をリセット
        initialization(&mode, &number_of_mines, mine_map, game_board, &finish_flag, &open_count);
        ...
        while(!finish_flag) {
            ...
            if (mode == 1) {
                ...
                // 近傍８セルの内、地雷とフラグのないセルを開ける
                    check_neighbors(x, y, mine_map, game_board, &open_count);
                }
            } 
...
void select_cell(int *mode, int *x, int *y) {
    ...
    do {
        ...
        printf("y : ");
        scanf("%d", y);
        if (!(*x >= 0 && *x < MS_SIZE && *y >= 0 && *y < MS_SIZE)) {
            printf("0~%dの範囲で入力してください。\n", MS_SIZE-1);
        }
    } while (!(*x >= 0 && *x < MS_SIZE && *y >= 0 && *y < MS_SIZE));
}
